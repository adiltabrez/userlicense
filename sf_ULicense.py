...
q_license_name = ("SELECT Name, UsedLicenses, TotalLicenses FROM UserLicense WHERE Name IN ('Salesforce Platform','Customer Community')")
q_org_name= "SELECT Name FROM Organization"
...
d_lic_df.insert(0,"Client Name","Universal Containers")
d_lic_df.rename(
    columns={'Name':'License Name','TotalLicenses':'Total Licenses','UsedLicenses':'Used Licenses'},
    inplace=True
)
d_lic_df = d_lic_df[['Client Name','License Name','Total Licenses','Used Licenses','Remaining Licenses','Profile Count']]
print(d_lic_df)
